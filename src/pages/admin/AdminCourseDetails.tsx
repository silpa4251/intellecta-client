...
const AdminCourseDetailsPage = () => {
  ...
  if (isLoading) return <div className="p-6">Loading course details...</div>;
  if (isError) return <div className="p-6">Error: {(error as Error).message}</div>;

  return (
    <div className="px-6 max-h-[75vh] overflow-y-auto relative scrollbar-hide">
      <div className="p-6 max-w-4xl mx-auto">
        ...
        {/* Course Details Card */}
        <div className="rounded-2xl bg-white border border-gray-200 p-6 shadow-sm dark:bg-gray-800">
          ...
          <div className="grid gap-4">
            <div>
              ...
              <p className="text-lg text-gray-800 dark:text-gray-200">
                {data?.course?.subject && data?.course?.subject.toLocaleUpperCase().charAt(0) + data?.course?.subject.slice(1)}
              </p>
            </div>
            <div>
            ...
            <div>
              <h3 className="text-sm font-medium text-gray-500">Number of Lessons</h3>
              <p className="text-lg text-gray-800 dark:text-gray-200">
                {data?.lessons?.length || 0}
              </p>
            </div>
            <div>
              <h3 className="text-sm font-medium text-gray-500">Lessons</h3>
              {data?.lessons && data.lessons.length > 0 ? (
                ...
                <p className="text-gray-600 dark:text-gray-300">No lessons available</p>
              )}
            </div>
          </div>
        </div>
